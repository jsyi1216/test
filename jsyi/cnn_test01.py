...
cx=pd.DataFrame({'x1':a,'x2':b,'x3':c})
...
cX_train,cX_test,y_train,y_test = train_test_split(cx,y,test_size=0.2,random_state=1)
lX_train,lX_test,y_train,y_test = train_test_split(lx,y,test_size=0.2,random_state=1)

# 1. CNN model for numeric data 


inp1 = Input(shape=(3,1), name="input1")
x1 = Conv1D(filters=16, kernel_size=3, strides=1, padding='valid', activation='relu', name='x1_conv_1')(inp1)
...
fx = Dense(1, activation='linear', name='fx')(x)

model = Model(inputs=[inp1, inp2], output=fx)
model.compile(loss='mse', optimizer='adam', metrics=[r_squared])
model.summary()

#history = model.fit(cX_train.values[:,:,np.newaxis], y_train, batch_size=256, epochs=100, validation_split=0.2, verbose=1)
#score = model.evaluate(cX_test.values[:,:,np.newaxis], y_test, verbose=0)
#pred = model.predict(cX_test.values[:,:,np.newaxis])
history = model.fit([cX_train.values[:,:,np.newaxis], lX_train.values[:,:,np.newaxis]], y_train, batch_size=256, epochs=100, validation_split=0.2, verbose=1)
score = model.evaluate([cX_test.values[:,:,np.newaxis], lX_test.values[:,:,np.newaxis]], y_test, verbose=0)
pred = model.predict([cX_test.values[:,:,np.newaxis], lX_test.values[:,:,np.newaxis]])
